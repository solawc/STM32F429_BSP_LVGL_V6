...
__align(32) uint8_t mem2base[MEM2_MAX_SIZE] __attribute__((at(0xD2000000)));					//外部SDRAM内存池,前面2M给LTDC用了(1280*800*2)
...
uint32_t mem2mapbase[MEM2_ALLOC_TABLE_SIZE] __attribute__((at(0xD2000000+MEM2_MAX_SIZE)));	    //外部SRAM内存池MAP
uint32_t mem3mapbase[MEM3_ALLOC_TABLE_SIZE] __attribute__((at(0X10000000+MEM3_MAX_SIZE)));	    //内部CCM内存池MAP
//内存管理参数	   
const uint32_t memtblsize[SRAM_BANK]={MEM1_ALLOC_TABLE_SIZE,MEM2_ALLOC_TABLE_SIZE,MEM3_ALLOC_TABLE_SIZE};	//内存表大小
const uint32_t memblksize[SRAM_BANK]={MEM1_BLOCK_SIZE,MEM2_BLOCK_SIZE,MEM3_BLOCK_SIZE};					//内存分块大小
