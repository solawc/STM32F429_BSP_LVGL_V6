 ...
 *   GLOBAL FUNCTIONS
 **********************/
static lv_color_t color_buf[LV_HOR_RES_MAX*LV_VER_RES_MAX] __attribute__((at(LV_MEM_ADR+LV_MEM_SIZE)));
void lv_port_disp_init(void)
{
    ...
    lv_disp_buf_init(&disp_buf_3, color_buf, NULL, LV_HOR_RES_MAX * LV_VER_RES_MAX);   /*Initialize the display buffer*/
...
static void disp_flush(lv_disp_drv_t * disp_drv, const lv_area_t * area, lv_color_t * color_p)
{
    ...
    DMA2D->CR = 0U; // MEM 2 MEM
    DMA2D->FGMAR = (uint32_t)color_p;  // add color origin addr
    DMA2D->OMAR = (uint32_t)Ltdc_Handler.LayerCfg[0].FBStartAdress + 2*(LV_HOR_RES_MAX*area->y1 + area->x1);
    DMA2D->FGOR = 0;
    DMA2D -> OOR = offline;
